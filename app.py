...
# ─── 3) Função para ler e tratar dados ────────────────────────────────────────
@st.cache_data
def load_data():
    ...
    df = pd.DataFrame({
        ...
        "Valor_raw":     df_raw["Valor"].str.replace(".", "").str.replace(",", ".").astype(float),
...
col2.metric("📝 Lançamentos",    f"{count}")
col3.metric("🎯 Ticket Médio",    f"R$ {ticket:,.2f}")

# ─── 8) Prepara AgGrid com seleção ────────────────────────────────────────────
...
# ─── 9) Formulário de edição ao lado ─────────────────────────────────────────
